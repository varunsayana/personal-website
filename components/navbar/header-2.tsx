...
import { Button, buttonVariants } from '@/components/navbar/ui/button';
import { cn } from '@/lib/utils';
import { MenuToggleIcon } from '@/components/navbar/menu-toggle-icon';
import { useScroll } from '@/components/navbar/use-scroll';
	...
	return (
		<header
			className={cn(
				...
				{
					'bg-black/80 supports-[backdrop-filter]:bg-black/50 border-border backdrop-blur-lg md:top-4 md:max-w-4xl md:shadow':
						scrolled && !open,
					'bg-black/90': open,
				},
			)}
		>
			...
			>
				...
				<div className="hidden items-center gap-2 md:flex">
					{links.map((link, i) => (
						<a key={i} className={buttonVariants({ variant: 'ghost', className: 'text-white' })} href={link.href}>
							{link.label}
						</a>
					))}
				...
				<Button size="icon" variant="outline" onClick={() => setOpen(!open)} className="md:hidden text-white">
			...
			<div
				className={cn(
					'bg-black/90 fixed top-14 right-0 bottom-0 left-0 z-50 flex flex-col overflow-hidden border-y md:hidden',
					open ? 'block' : 'hidden',
				)}
			>
				<div
